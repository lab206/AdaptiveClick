...
class AdaptiveFocalLossSigmoid(nn.Module):
    def __init__(self, axis=-1, gamma=2, alpha=1.0, delta=0.4, max_mult=-1, eps=1e-12,
                 from_sigmoid=False, detach_delimeter=True,
                 batch_axis=0, weight=None, size_average=True,
                 ignore_label=-1):
        super(AdaptiveFocalLossSigmoid, self).__init__()
        self._axis = axis
        self._gamma = gamma
        self._alpha = alpha
        self._delta = delta
    ...
    def forward(self, pred, label):
        ...
        pos_sum = torch.sum(torch.where(one_hot, pt, 0), dim=(-2, -1), keepdim=True)
        num_sum = torch.sum(torch.where(one_hot, 1, 0), dim=(-2, -1), keepdim=True)
        gamma = self._gamma + (1 - (pos_sum / (num_sum + self._eps)))
        if self._detach_delimeter:
            gamma = gamma.detach()

        beta = (1 - pt) ** gamma
        sw_sum = torch.sum(sample_weight, dim=(-2, -1), keepdim=True)
        beta_sum = (1 + self._delta * gamma) * torch.sum(beta, dim=(-2, -1), keepdim=True)
        ...
        loss = - mult * beta * torch.log(
            torch.min(pt + self._eps, torch.ones(1, dtype=torch.float).to(pt.device))
        ) + self._alpha * (1 - pt) ** (gamma + 1)
