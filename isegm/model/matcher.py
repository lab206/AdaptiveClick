...
def batch_sigmoid_adaptive_focal_loss(inputs, targets, epsilon: float = 0.5, gamma: float = 2,
                                      delta: float = 0.4, alpha: float = 1.0, eps: float = 1e-12):
    if targets.shape[0] == 0:
        return batch_sigmoid_focal_loss(inputs, targets)

    hw = inputs.shape[1]
    prob = inputs.sigmoid()
    p_t = torch.einsum("nc,mc->nm", prob, targets) + torch.einsum("nc,mc->nm", 1 - prob, (1 - targets))
    p_t = p_t / hw
    one_hot = targets > 0.5
    with torch.no_grad():
        p_sum = torch.einsum("nc,mc->nm", prob, targets)
        ps_sum = torch.sum(torch.where(one_hot, 1, 0), dim=-1, keepdim=True).transpose(1, 0)
    ...
    # [n, hw] [n, m]
    focal_pos = F.binary_cross_entropy_with_logits(
        inputs, torch.ones_like(inputs), reduction="none"
    )
    focal_neg = F.binary_cross_entropy_with_logits(
        inputs, torch.zeros_like(inputs), reduction="none"
    )
    assert epsilon is 0.5
    if epsilon >= 0:
        # mult [10, m] prob [10, hw] gamma [10, m] focal_pos [10, hw]
        focal_pos = focal_pos * epsilon
        focal_neg = focal_neg * (1 - epsilon)

    loss = torch.einsum("nc,mc->nm", focal_pos, targets) + torch.einsum("nc,mc->nm", focal_neg, (1 - targets))
    loss = mult * (1 - p_t) ** gamma * loss / hw + epsilon * alpha * ((1 - p_t) ** gamma + 1)
    return loss
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            with autocast(enabled=False):
                ...
                # TODO：Use AFL for matching
                cost_mask = batch_sigmoid_adaptive_focal_loss(out_mask, tgt_mask)
