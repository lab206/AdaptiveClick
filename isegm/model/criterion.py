...
from isegm.utils.misc import is_dist_avail_and_initialized, nested_tensor_from_tensor_list, get_world_size
...
def sigmoid_normalized_focal_loss(inputs, targets, num_masks, alpha: float = 0.25, gamma: float = 2):
    """
    Args:
        inputs: A float tensor of arbitrary shape.
                The predictions for each example.
    ...
    with torch.no_grad():
        ...
        beta_sum = torch.sum(beta, dim=-1, keepdim=True) + 1e-12
        mult = sw_sum / beta_sum

    loss = mult * ce_loss * beta
...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        losses = {
            "loss_mask": sigmoid_adaptive_focal_loss(point_logits, point_labels, num_masks),
    ...
    def _get_binary_mask(self, target):
        ...
        target_index = target.unsqueeze(0).cpu().long()
        target_onehot = target_onehot.scatter(dim=0, index=target_index, value=1)
    ...
    def _get_targets(self, gt_masks):
        ...
        for mask in gt_masks:
            ...
            cls_label = torch.unique(mask).long()
            labels = cls_label[1:]
